...
Don't like running stuff as root? Run it in docker. You can customize the crossdomain.xml file if you need (in the /docker directory) and then rebuild with sh build.sh.

To run after you've built;

```bash
docker run flash_policy_server
```
